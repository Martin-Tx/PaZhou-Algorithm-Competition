...
def inference_on_cls_dataset(model, data_loader, evaluator, scales = [0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.4],
                             flip_test=True, moe_group=None):
    """
    ...
    with paddle.no_grad():
        for idx, inputs in enumerate(data_loader):
            ...
            # scales = [1.0]
            im = inputs['fgvc']["image"]
            h_input, w_input = im.shape[-2], im.shape[-1]

            nums = len(scales) * 2 if flip_test else len(scales)
            # print(f'nums: {nums}')
            final_results = None
            for scale in scales:
                ...
                inputs['fgvc']["image"] = im

                ori_outputs = model(inputs, moe_group) if moe_group is not None else model(inputs)
                if final_results == None:
                    final_results = copy.deepcopy(ori_outputs)
                else:
                    final_results['fgvc'] = final_results['fgvc'] +  ori_outputs['fgvc']

                if flip_test:
                    # raise NotImplementedError
                    # print(inputs)
                    # inputs['fgvc']["image"] = inputs['fgvc']["image"].flip(dims=[3])
                    inputs['fgvc']["image"] = inputs['fgvc']["image"][:, :, :, ::-1]
                    flip_outputs = model(inputs, moe_group) if moe_group is not None else model(inputs)
                    final_results['fgvc'] = final_results['fgvc'] + flip_outputs['fgvc']

            final_results['fgvc'] = final_results['fgvc'] / nums

            # TODO cuda.synchronize()
            total_compute_time += time.perf_counter() - start_compute_time
            evaluator.process(inputs, final_results)
