...
class MultiTaskBatchFuse(nn.Layer):
    ...
    def __init__(
        ...
        if pretrained:
            ...
            self.backbone.set_dict(param_state_dict)
            missing_keys = []
            unexpected_keys = []
            for k, v in self.backbone.state_dict().items():
                if k not in param_state_dict.keys():
                    missing_keys.append(k)
            for k, v in param_state_dict.items():
                if k not in self.backbone.state_dict().keys():
                    unexpected_keys.append(k)
            print(f"backbone missing keys: {missing_keys}")
            print(f"backbone unexpected keys: {unexpected_keys}")
            print('*' * 50)
        # head
        ...
        # param_state_dict = paddle.load(pretrain_path)
        # print(self.heads[self.task2head_mapping['segmentation']].keys())
        # print(self.heads[self.task2head_mapping['segmentation']])

        # print(param_state_dict)
        # self.heads[self.task2head_mapping['segmentation']].set_dict(param_state_dict)
    ...
    def forward(self, task_batched_inputs):
        ...
        for task_name, batched_inputs in task_batched_inputs.items():
            ...
            if self.training:
                ...
                # task_name fgvc trafficsign segmentation
