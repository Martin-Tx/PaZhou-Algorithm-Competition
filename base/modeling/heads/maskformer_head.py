...
class MaskFormer(nn.Layer):
    ...
    def __init__(self,
        ...
        self.seghead = MaskFormerHead(output_shape, num_classes)

        self.pretrained = pretrained
        self.init_weight()
        # print(f'backbone.output_shape(): {backbone.output_shape()}')
    ...
    def forward(self, features):
        for idx, f in enumerate(self.in_features[::-1]):
            ...
            if lateral_conv is None:
                ...
                y = cur_fpn + F.interpolate(
                    y, size=cur_fpn.shape[-2:], mode='bilinear')
