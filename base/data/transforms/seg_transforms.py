...
class Compose:
    ...
    def __call__(self, data):
        ...
        for op in self.transforms:
            data = op(data)
        # print(f'segmentation data_time: {time.time() - start} s')
...
class ResizeStepScaling:
    ...
    def __call__(self, data):
        ...
        data['image'] = functional.resize(data['image'], (w, h), cv2.INTER_LINEAR)
        for key in data.get('gt_fields', []):
            data[key] = functional.resize(data[key], (w, h), cv2.INTER_NEAREST)

        # print(f'resize data_time: {time.time() - start} s')
        return data
...
class Normalize:
    ...
    def __call__(self, data):
        ...
        # label_img_1 = label * 0.4 + img
        # name = str(time.time())
        # save_dir = 'vis_seg'
        # if not os.path.exists(save_dir):
        #     os.mkdir(save_dir)
        # cv2.imwrite(os.path.join(save_dir, name + '_1.png'), label_img_1[...,::-1])
        # cv2.imwrite(os.path.join(save_dir, name + '_2.png'), img)
...
class RandomPaddingCrop:
    ...
    def __call__(self, data):
        ...
        # print(f'label.shape: {label.shape}')
        # label = cv2.applyColorMap(label,cv2.COLORMAP_JET)
        # label_img_1 = label * 0.8 + img
        # print(5)
        # name = str(time.time())
        # print(6)
        # save_dir = '/home/Lian.qy/HHD2/Open-TransMind/PAZHOU/base/vis_3'
        # print(7)
        # cv2.imwrite(os.path.join(save_dir, name + '_1.png'), label_img_1)
        # cv2.imwrite(os.path.join(save_dir, name + '_2.png'), img)
        # print(8)
        # print(f'crop data_time: {time.time() - start} s')
        return data
...
class RandomDistort:
    ...
    def __call__(self, data):
        ...
        end1 = time.time()
        # print(f'distort1 time: {end1 - start} s')
        if data['image'].ndim > 2:
        ...
        # print(f'distort4 time: {end4 - end3} s')
        
        data['image'] = np.asarray(data['image']).astype('float32')
        # print(f'distort time: {time.time() - start} s')
        return data
...
class GenerateInstanceTargets:
    ...
    def __call__(self, data):
        ...
        # a = paddle.take_along_axis(a,b,axis=0)
        ...
        if 'label' in data:
            ...
            # print(f'generate data_time: {time.time() - start} s')
        return data
